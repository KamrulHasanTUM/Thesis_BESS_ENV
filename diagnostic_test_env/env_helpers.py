...
def create_bess_action_space(env):
    ...
    """
    # CRITICAL FIX (Nov 2, 2025): Action space MUST be normalized [-1, +1] for PPO
    # PPO and other RL algorithms expect normalized action spaces for proper exploration
    # The actions will be scaled to MW range by apply_bess_action() function
    # This prevents the 100% action clipping bug that was blocking learning
    action_space = Box(
...
def create_bess_observation_space(net, num_bess, bess_power_mw, voltage_min_pu=0.5, voltage_max_pu=1.5):
    ...
    observation_spaces = {
        # ========== FEATURE SELECTION (Section 12.7) ==========
        # REMOVED: "discrete_switches" (422 features, mostly static)
        #   - Switches rarely change during BESS operation (topology is fixed)
        #   - High dimensionality with low information content
        #   - Removing saves ~422 features
        # REMOVED: "continuous_vm_bus" (378 features, voltage not critical for congestion)
        #   - Primary objective is congestion management (line loading), not voltage control
        #   - Voltage information is implicit in line loadings
        #   - BESS actions primarily affect real power flow, not voltage
        #   - Removing saves ~378 features
        # TOTAL REDUCTION: ~800 features removed (47% dimensionality reduction)

        # ========== Grid Observations (from ENV_RHV) - KEPT ==========
        # "discrete_switches": discrete_space,  # REMOVED (Section 12.7)
        # "continuous_vm_bus": Box(...),        # REMOVED (Section 12.7)
        ...
        # ========== OBSERVATION ENHANCEMENTS (from Section 12.1-12.3 of report) ==========
        # Priority 1.1: Top-K congested lines feature
        # - Explicitly shows the 10 most congested lines to agent
        # - Agent focuses learning on critical congestion points
        # - Reduces noise from observing all 270 lines equally
        ...
        # Priority 1.2: BESS-to-line sensitivity features
        # - Shows how each BESS affects each of the top-K congested lines
        # - Agent learns "If I discharge BESS 3, line 127 loading decreases by X%"
        # - Provides explicit causality information missing from current implementation
        ...
        # Priority 1.3: Loading trend (temporal context)
        # - Shows change in loading from previous timestep
        # - Enables anticipatory control: Agent sees if congestion is increasing
        # - Helps predict future grid state
        "loading_trend": Box(
            low=-100.0,
            high=100.0,
            shape=(num_lines,),
        ...
        # ========== PHASE 1 IMPROVEMENT 2: SoC Headroom Features ==========
        # These features explicitly tell agent how much charge/discharge capacity remains
        # CRITICAL for learning SoC constraints

        # Charge headroom: How much can we charge before hitting soc_max?
        # - 0.0 = At maximum SoC (90%), cannot charge more
        # - 1.0 = At minimum SoC (10%), maximum charge capacity available
        # - Agent learns: "If charge_headroom near 0, don't command more charging"
        ...
        # - 0.0 = At minimum SoC (10%), cannot discharge more
        # - 1.0 = At maximum SoC (90%), maximum discharge capacity available
        # - Agent learns: "If discharge_headroom near 0, don't command discharge"
        ...
        # Normalized SoC: Alternative representation of SoC
        # - 0.0 = At soc_min (10%)
        # - 1.0 = At soc_max (90%)
        # - Provides explicit mapping to operational range
...
def compute_bess_line_sensitivities(env, top_k_line_indices):
    ...
    This function implements finite-difference approximation of Power Transfer Distribution Factors (PTDF).
    For each BESS unit, it perturbs the power injection by +1 MW and measures the change in
    line loading for the top-K congested lines.

    Sensitivity = ∂(line_loading) / ∂(BESS_power)

    Physical Interpretation:
    - Positive sensitivity: BESS discharge increases line loading (worsens congestion)
    - Negative sensitivity: BESS discharge decreases line loading (relieves congestion)
    - Large absolute value: BESS has strong electrical influence on that line
    - Near-zero: BESS has minimal impact on that line (electrically distant)

    Args:
        env: Environment instance with pandapower network
        top_k_line_indices: Indices of the K most congested lines

    Returns:
        np.ndarray: Shape (num_bess, K) - Sensitivity matrix
                   sensitivities[i, j] = effect of BESS i on line j
    """
    import pandapower as pp

    sensitivities = np.zeros((env.num_bess, len(top_k_line_indices)), dtype=np.float32)

    # Store baseline loading before perturbation
    baseline_loading = env.net.res_line.loc[top_k_line_indices, 'loading_percent'].values.copy()

    for i in range(env.num_bess):
        # Get original power of BESS i
        original_p = env.net.sgen.at[env.bess_sgen_indices[i], 'p_mw']

        # Perturb BESS i by +1 MW (small perturbation for linear approximation)
        ...
        try:
            ...
            # Compute sensitivity: Δloading / Δpower
            # Division by 1.0 MW is implicit (perturbation = 1 MW)
            sensitivities[i, :] = perturbed_loading - baseline_loading

        except Exception as e:
            # If power flow fails with perturbation, sensitivity is zero (no reliable info)
            print(f"Warning: Power flow failed for BESS {i} sensitivity computation: {e}")
            sensitivities[i, :] = 0.0

        # Restore original power
        env.net.sgen.at[env.bess_sgen_indices[i], 'p_mw'] = original_p

    # Restore grid state with original BESS powers
    try:
        pp.runpp(env.net, algorithm='nr', max_iteration=100)
    except Exception as e:
        print(f"Warning: Failed to restore grid state after sensitivity computation: {e}")
...
def build_observation_from_grid_state(env):
    ...
    # vm_pu = env.net.res_bus['vm_pu'].fillna(0).values.astype(np.float32)  # REMOVED (Section 12.7)

    # discrete_switches = np.concatenate([  # REMOVED (Section 12.7)
    #     env.net.switch['closed'].astype(int).values,
    #     env.net.line['in_service'].astype(int).values
    # ])

    # Build base observation with grid state
    # FEATURE SELECTION (Section 12.7): Removed discrete_switches and continuous_vm_bus
    observation = {
        # "discrete_switches": discrete_switches,  # REMOVED (Section 12.7)
        # "continuous_vm_bus": vm_pu,              # REMOVED (Section 12.7)
    ...
    if hasattr(env, 'bess_soc') and hasattr(env, 'bess_power'):
        ...
        # ========== OBSERVATION ENHANCEMENTS (Priority 1.1 - 1.3) ==========

        # Priority 1.1: Top-K congested lines
        # Extract indices of 10 most congested lines
        top_k_indices = np.argsort(loading_percent)[-10:][::-1]  # Sort descending, take top 10
        observation["top_k_congested_lines"] = loading_percent[top_k_indices]

        # Priority 1.2: BESS sensitivity to top-K lines
        # Only compute if BESS sgen elements exist (after first action)
        if hasattr(env, 'bess_sgen_indices') and env.bess_sgen_indices is not None:
            ...
            # Before first action, fill with zeros
            observation["bess_sensitivity_to_top_k"] = np.zeros((env.num_bess, 10), dtype=np.float32)
        ...
        if hasattr(env, 'loading_history') and len(env.loading_history) >= 1:
            ...
            # First timestep: no history, trend is zero
            observation["loading_trend"] = np.zeros_like(loading_percent, dtype=np.float32)

        # ========== PHASE 1 IMPROVEMENT 2: SoC Headroom Observations ==========
        # Explicitly show agent how much charge/discharge capacity is available
        # This is CRITICAL for agent to learn SoC constraints

        # Charge headroom: How much can we charge before hitting soc_max?
        # Formula: (soc_max - current_soc) / (soc_max - soc_min)
        # Result: 0.0 = at max (can't charge), 1.0 = at min (maximum charge capacity)
        ...
        # Result: 0.0 = at min (can't discharge), 1.0 = at max (maximum discharge capacity)
        ...
        # Normalized SoC: Alternative representation mapping operational range to [0, 1]
        # Formula: (current_soc - soc_min) / (soc_max - soc_min)
        # Result: 0.0 = at soc_min (10%), 1.0 = at soc_max (90%)
...
def calculate_bess_reward(env, max_loading_before, max_loading_after):
    ...
    # ========== 3. CRITICAL: Action Feasibility Scaling (Phase 1, Improvement 1) ==========
    # Problem: Agent gets punished for consequences of constraints, not choices
    # Solution: Scale congestion reward by action feasibility
    #
    # When BESS at 10% SoC:
    # 1. Agent commands: "Discharge 50 MW"
    # 2. Reality: SoC constraint clips to 0 MW discharge
    # 3. Loading increases (no relief)
    # 4. OLD: Agent learns "that grid state was bad" (WRONG!)
    # 5. NEW: Agent learns "don't discharge when SoC is low" (CORRECT!)
    #
    # Implementation: Compare intended SoC change vs actual SoC change
    action_feasibility = np.ones(env.num_bess)  # Default: fully feasible

    if hasattr(env, 'soc_before_action') and env.soc_before_action is not None:
        for i in range(env.num_bess):
            # Calculate intended SoC after this timestep's power action
            intended_soc = env.soc_before_action[i] - (env.bess_power[i] * env.time_step_hours) / env.bess_capacity_mwh
            actual_soc = env.bess_soc[i]

            # If there's a significant difference, action was clipped
    ...
    # Average feasibility across all BESS
    avg_feasibility = np.mean(action_feasibility)

    # Scale congestion reward by feasibility
    # - If fully feasible (1.0): full congestion reward
    # - If partially feasible (0.5): 50% congestion reward
    # - If infeasible (0.0): no congestion reward (action didn't execute)
    feasibility_scaled_congestion = congestion_reward * avg_feasibility

    # ========== 4. CRITICAL: Infeasibility Penalty (Phase 1, Improvement 1) ==========
    # Penalize actions that get clipped due to SoC constraints
    # This teaches agent: "Plan actions that can actually be executed"
    infeasibility_penalty = -50.0 * (1.0 - avg_feasibility)

    # ========== 5. CRITICAL: SoC At Bounds Penalty (Phase 1, Improvement 1) ==========
    # Additional penalty for operating right at SoC bounds
    # This is STRONGER than the existing boundary margin penalty
    ...
    # ========== 6. EXPLORATION: Action Magnitude Bonus (Keep for exploration) ==========
    # Keep the action magnitude bonus to encourage exploration
    ...
    # ========== Total Reward (Phase 1 Modified) ==========
    # OLD: total_reward = congestion_reward + soc_penalty + action_magnitude_bonus
    # NEW: Scale congestion by feasibility, add infeasibility penalty
    total_reward = (
        feasibility_scaled_congestion +  # Scaled by action feasibility
        soc_penalty +                     # Existing boundary margin penalty
        infeasibility_penalty +           # NEW: Penalty for clipped actions
        soc_bounds_penalty +              # NEW: Penalty for being AT bounds
        action_magnitude_bonus            # Keep for exploration
    )

    # Breakdown for logging/debugging
    reward_breakdown = {
      'congestion': float(congestion_reward),
      'feasibility_scaled_congestion': float(feasibility_scaled_congestion),
      'soc_penalty': float(soc_penalty),
      'action_feasibility': float(avg_feasibility),
      'infeasibility_penalty': float(infeasibility_penalty),
      'soc_bounds_penalty': float(soc_bounds_penalty),
