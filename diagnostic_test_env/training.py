...
def create_model(env, training_config, logdir):
    ...
    from torch import nn
      ...
      # Force wider exploration by initializing policy with higher std
    policy_kwargs = dict(
      log_std_init=-1.0,  # Default is -2.0, this gives 2.7x more exploration
      net_arch=[dict(pi=[256, 256], vf=[256, 256])]  # Larger network for better learning
  )

    return PPO(
      "MultiInputPolicy",
      env,
      policy_kwargs=policy_kwargs,  # Add this line
      verbose=0,
...
def train_model_with_wandb(model, training_config, env_config):
    """Train model with W&B tracking including congestion analysis."""
    full_config = {**env_config, **training_config}
    run = init_wandb_run(full_config, project_name="thesis-bess-env")
    
    wandb_callback = WandbCallback(log_freq=10)  # ← Changed from log_freq=100
    
    try:
        model.learn(
            total_timesteps=training_config['total_timesteps'],
            callback=wandb_callback,  # Single callback with all features
            progress_bar=True
        )
